...
def create():
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            topicid = db.session.execute(
                select(Topic.topic_id).where(Topic.topic_name == topic)
            ).fetchone()
            if not topicid:
                topicdb = Topic(topic_name=topic)
                db.session.add(topicdb)
            db.session.query(User).where(User.id == g.user[0].id).update(
                {User.posts_created: User.posts_created + 1}
            )
            db.session.execute(
                insert(Post).values(
                    author_id=int(g.user[0].id),
                    title=title,
                    body=body,
                    topic_id=topicid[0],
                )
...
def get_post(id, check_author=True):
    post = db.session.execute(
        select(Post, User).join(User, Post.author_id == User.id).where(Post.id == id)
    ).fetchone()
...
def update(id):
    ...
    if request.method == "POST":
        ...
        if error is not None:
            ...
            topic_id = db.session.execute(
                select(Topic.topic_id).where(Topic.topic_name == topic)
            ).fetchone()
            db.session.query(Post).where(Post.id == id).update(
                {Post.title: title, Post.body: body, Post.topic_id: topic_id[0]}
            )
            db.session.query(User).where(User.id == g.user[0].id).update(
                {User.posts_updated: User.posts_updated + 1}
            )
            db.session.commit()
            return redirect(url_for("blog.index"))
...
def post(post_id):
    post = db.session.execute(
        select(Post).join(User, Post.author_id == User.id).where(Post.id == post_id)
    ).fetchone()
    post = get_post(post_id, check_author=False)
    comments = db.session.execute(
        text(
            "SELECT com.*, usr.* FROM comments com JOIN user usr ON com.user_id == usr.id WHERE com.post_id = :post_id"
        ),
        {"post_id": post_id},
    ).fetchall()
    if request.method == "POST":
        ...
        db.session.execute(
            insert(Comments).values(
                user_id=g.user[0].id, comment_body=commentbody, post_id=post_id
            )
        )
        db.session.commit()
        comments = db.session.execute(
